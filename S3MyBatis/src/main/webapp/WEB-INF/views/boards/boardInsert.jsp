...
<head>
	...
	<script type="text/javascript">
		...
		var gotoBoardDetail = function() {
			location.href='boardDetail.do?tbid=${param.tbid}&bbsno=${param.ref_no}&pg=${param.pg}&field=${param.field}&query=${param.query}';
		};	
		...
		var insert = function() {
			...
			$.ajax({
				type : "POST",
				url : "boardInsertProc.do",
				data : {
					bbsno     :  $("#bbsno").val(),
					ref_no	  :  $("#ref_no").val(),
					ref_step  :  $("#ref_step").val(),
					ref_level :  $("#ref_level").val(),
					title     :  $("#title").val(),
					cont      :  $("#cont").val(),
					writer	  :  $("#writer").val(),
					likes     :  $("#likes").val(),
				...
				success : function(data) {
					if(data.ok=='Y'){
						location.href="boardList.do?tbid="+$("#tbid").val()+"&bbsno="+$("#bbsno").val();
					}
					if(data.ok=='N'){
						alert(msg);
						location.reload();
		...
		<div class="container">
			<form id="submitFrm" method="post" action="">
				<table class="table table-bordered">
					<tr>
						<td class="info">Title</td><td colspan="5"><input type="text" id="title" name="title" class="form-control" value="" /></td>
					</tr>
					<!-- <tr>
						<td class="info">Writer</td><td></td>
						<td class="info">Reg. Date</td><td></td>
						<td class="info">View</td><td></td>
					</tr> 
					-->

					<tr>
						<td colspan="6" class="active">
							<textarea id="cont" name="cont" class="form-control" rows="10"></textarea>
							<script type="text/javascript">
						       var ckeditor_config = {
						    		   height : '450px',
				...
				<input type="hidden" id="ref_no" name="ref_no" value="${ref_no}" />
				<input type="hidden" id="ref_step" name="ref_step" value="${ref_step}" />
				<input type="hidden" id="ref_level" name="ref_level" value="${ref_level}" />
				...
				<input type="hidden" id="ref_parent" name="ref_parent" value="${ref_parent}" />
	        ...
	        <div align="right">
					...
					<c:if test="${ref_no!=0}">
						<a href="javascript:gotoBoardDetail();" class="btn btn-default btn-sm">Back to Detail</a>
					</c:if>
