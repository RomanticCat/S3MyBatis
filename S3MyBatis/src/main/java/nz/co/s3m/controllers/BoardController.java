	...
	public String boardEdit(Model model, int bbsno, String tbid, HttpServletRequest request) {
		...
		if( (vo.getaFile() != null) && (! vo.getaFile().trim().equals("")) ) {
			...
			while(st.hasMoreElements()){
				...
				String removeFileLink = "<button type='button' class='btn btn-warning btn-xs' id='removeAFileBtn"
						+ idx +"' onclick='javascript:removeAFileButton(\"aFile\", \"removeAFileBtn"+ idx +"\", \"" 
						+ storedFile+"$$$\");'><span class='glyphicon glyphicon-trash'></span> &nbsp; " + orgFile+ "</button>";
